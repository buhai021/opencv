...
img_hsv=cv2.cvtColor(img,cv2.COLOR_BGR2HSV)         #将图片由BGR转为HSV
...
#canny算子
image_canny80_200=cv2.Canny(img_gray,80,150)
image_canny100_200=cv2.Canny(img_gray,100,200)
plt.subplot(131),plt.imshow(image_canny80_200),plt.title('Canny50_100'),plt.axis('off')
...
plt.show()
cv2.imshow('image',image_canny80_200)
...
'''
#提取轮廓
_,threshold_image=cv2.threshold(image_canny80_200,127,255,cv2.THRESH_BINARY)#忽略实际使用的阈值
